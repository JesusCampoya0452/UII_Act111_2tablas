...
def detalle_producto(request, producto_id):
    producto = get_object_or_404(Producto, ID_prd=producto_id)
...
def crear_producto(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('app_inventario:listar_productos')
    else:
        form = ProductoForm()
...
def editar_producto(request, producto_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('app_inventario:detalle_producto', producto_id=producto.ID_prd)
    else:
